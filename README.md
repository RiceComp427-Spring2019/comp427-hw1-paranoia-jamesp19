...
- Threats:
  - The first of two threats to protect against are "scripters"- people whose main goal is to modify the game data during gameplay. They might do this by sending forged packets or a modified game client to give their character unfair advantages. A scripter left unchecked could ruin the game for other players by killing them repeatedly to prevent them from being able to play, or removing all of their items. We want to ensure that people like this are not allowed to ruin the fun for others. Note that for scripters, integrity is a more important priority to protect than confidentiality. We don't mind if people read the game data- indeed, it would be impossible to play the game without seeing your character's information! However, we do care if a scripter is able to modify the game beyond the limits of what the game normally allows.
  - The second threat to protect against are "hackers"- people whose main goal is to read account data. Unlike game data, hackers would attempt to breach data servers directly, not through the game client. If they were able to do so and read the data, they could then sell that data to others, and the emails, passwords, and credit card information of hundreds of thousands of people would be compromised. We want to ensure that such hackers don't get the chance. Here, confidentiality is much more important than integrity. A hacker receives no benefit from being able to change a random player's credit card information, email, or password if they aren't able to read it beforehand, especially since the game can simply email the player's last known email indicating that a change was made. However, if they are able to read the data and release it into the wild, it would have major consequences.
- Countermeasures:
  - The first countermeasure, meant to be used against scripters, would 
