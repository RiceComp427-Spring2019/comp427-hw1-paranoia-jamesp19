...
- Countermeasures:
  ...
  - The countermeasure against hackers would be to hash the data in the servers with some kind of hashing algorithm. The benefit of this countermeasure is that a would-be hacker who wants read the data in the servers would not actually be able to decipher that data, even if they were able to access the server. Even if the algorithm used to hash the data was known by the scripter, if the game company never releases the unencrypted format of the data, and ensures that every data packet has unique data (via something like a timestamp), it would be near impossible for a hacker to deduce the correct format of the data that they should input into the hashing algorithm. The cost of such a countermeas.
